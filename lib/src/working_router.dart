...
class WorkingRouter<ID> extends ChangeNotifier
  ...
  void syncBackAfterNavigatorPop() {
    _routeBackUntil(
      (it) => !it.shouldBeSkippedOnRouteBack,
      reason: RouteTransitionReason.systemPopSync,
      applyTransitionDecision: false,
    );
  }
  ...
  void _routeBackUntil(
    ...
    required bool applyTransitionDecision,
  }) {
    final data = nullableData!;
    ...
    _routeTo(
      ...
      reason: reason,
      applyTransitionDecision: applyTransitionDecision,
    );
  }
  ...
  void _routeTo({
    ...
    WorkingRouterData<ID>? resolvedData = targetData;
    if (applyTransitionDecision) {
      resolvedData = _resolveTransitionDecision(
        oldData: oldData,
...
enum RouteTransitionReason {
  ...
  /// The Navigator already popped a route and router state is catching up.
  systemPopSync,
}
